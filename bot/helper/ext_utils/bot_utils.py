...
def bt_selection_buttons(id_):
    ...
    buttons.url("sᴇʟᴇᴄᴛ ғɪʟᴇs 🗳️", f"{base_url}/app/files/{id_}")
    buttons.callback("ᴘɪɴᴄᴏᴅᴇ 📌", f"btsel pin {gid} {pincode}")
    buttons.callback("ᴄᴀɴᴄᴇʟ ❌", f"btsel rm {gid} {id_}")
    buttons.callback("ᴅᴏɴᴇ sᴇʟᴇᴄᴛɪɴɢ ✅", f"btsel done {gid} {id_}")
...
def get_readable_message():
    ...
    for download in list(download_dict.values())[
        ...
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_PROCESSING]:
            ...
            msg += f"<b>{download.status()}:</b> » {download.progress()}"
